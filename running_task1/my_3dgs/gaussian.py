"""3D Gaussian model with all trainable parameters."""
...
class GaussianModel(nn.Module):
    ...
    def __init__(self, sh_degree=3):
        ...
        # Parameters (will be created in create_from_pcd)
        self._xyz = None          # positions
        self._features_dc = None  # SH DC (zeroth order)
        self._features_rest = None  # SH higher order
        self._scaling = None      # log-scale
        self._rotation = None     # quaternion
        self._opacity = None      # logit opacity
    ...
    def covariance_activation(self, scales, scaling_modifier, rotations):
        """Build 3D covariance from scales and quaternion rotations."""
        ...
        # Return upper triangular (symmetric 3x3 → 6 values)
        return torch.stack([
            cov[:, 0, 0], cov[:, 0, 1], cov[:, 0, 2],
            cov[:, 1, 1], cov[:, 1, 2], cov[:, 2, 2],
        ], dim=-1)

    def create_from_pcd(self, pcd_points, pcd_colors, spatial_lr_scale):
        ...
        # SH DC: convert RGB to SH zeroth order
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd_colors)).float().cuda())
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()
        features[:, :3, 0] = fused_color
        features[:, 3:, 1:] = 0.0

        # Scales: distance to nearest 3 neighbors
        dist2 = torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(pcd_points)).float().cuda()), 0.0000001)
        ...
        # Opacity: initialize as mostly opaque
        opacities = self.inverse_opacity_activation(0.1 * torch.ones((N, 1)).cuda())
        ...
        self.max_radii2D = torch.zeros((N)).cuda()

    def training_setup(self, training_args):
        """Setup optimizer with per-parameter learning rates."""
        self.percent_dense = training_args.percent_dense
        self.xyz_gradient_accum = torch.zeros((self._xyz.shape[0], 1)).cuda()
        self.denom = torch.zeros((self._xyz.shape[0], 1)).cuda()
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest,
        ...
        self.xyz_gradient_accum = torch.zeros((self._xyz.shape[0], 1)).cuda()
        self.denom = torch.zeros((self._xyz.shape[0], 1)).cuda()
        self.max_radii2D = torch.zeros((self._xyz.shape[0])).cuda()

    def cat_tensors_to_optimizer(self, tensors_dict):
        """Add new tensors to optimizer state."""
        optimizable_tensors = {}
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        """Densify (split/clone) and prune low-opacity Gaussians."""
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        """Split large Gaussians with high gradient into N smaller ones."""
        ...
        means = torch.zeros((stds.size(0), 3)).cuda()
        ...
        R_mat = torch.stack([
            ...
            2*(x*y + r*z), 1 - 2*(x*x + z*z), 2*(y*z - r*x),
            2*(x*z - r*y), 2*(y*z + r*x), 1 - 2*(x*x + y*y),
        ], dim=-1).reshape(-1, 3, 3)  # [K, 3, 3]
        R_mat = R_mat.repeat(N, 1, 1)  # [K*N, 3, 3]

        new_xyz = torch.bmm(R_mat, samples.unsqueeze(-1)).squeeze(-1) + \
        ...
        # Remove original large Gaussians
        prune_filter = torch.cat((
            selected_pts_mask,
            torch.zeros(N * selected_pts_mask.sum()).cuda(),
        ))
    ...
    def reset_opacity(self):
        """Reset opacity to low values (for periodic reset)."""
