...
def transform2origin(position_tensor):
    ...
    scale = 1.0 / max_diff
    original_mean_pos = original_mean_pos.to(device=position_tensor.device)
    scale = scale.to(device=position_tensor.device)
...
def generate_rotation_matrix(degree, axis):
    ...
    return rotation_matrix.to(degree.device)
...
def apply_rotation(position_tensor, rotation_matrix):
    rotation_matrix = rotation_matrix.to(device=position_tensor.device)
    rotated = torch.mm(position_tensor, rotation_matrix.T)
    return rotated


def apply_cov_rotation(cov_tensor, rotation_matrix):
    rotation_matrix = rotation_matrix.to(device=cov_tensor.device)
...
def shift2center111(position_tensor):
    tensor111 = torch.tensor([1.0, 1.0, 1.0], device=position_tensor.device)
