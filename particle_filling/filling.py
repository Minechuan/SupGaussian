...
def get_particle_volume(pos, grid_n: int, grid_dx: float, unifrom: bool = False):
    pos = pos.reshape(-1, 3)
    device = pos.device
    grid_idx = torch.floor(pos / grid_dx).long()
    grid_idx = torch.clamp(grid_idx, min=0, max=grid_n - 1)

    linear_idx = (
        grid_idx[:, 0] * grid_n * grid_n + grid_idx[:, 1] * grid_n + grid_idx[:, 2]
    )
    counts = torch.zeros(grid_n * grid_n * grid_n, dtype=torch.float32, device=device)
    counts.scatter_add_(0, linear_idx, torch.ones_like(linear_idx, dtype=torch.float32))
    particle_vol = (grid_dx * grid_dx * grid_dx) / counts[linear_idx].clamp_min(1.0)

    if unifrom:
        return torch.mean(particle_vol).repeat(pos.shape[0])
    return particle_vol
